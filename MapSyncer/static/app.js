...
function downloadSeq(sequenceId, to_path = 0) {
    ...
    fetch('/download-sequence', {
        ...
        if (response.ok) {
            handleButtonState(button, [buttonSuccessClass, buttonDisabledClass], [buttonDisabledClass, buttonWarningClass], 'Downloaded')
...
function uploadSequence(sequenceId, button, to_path) {
    ...
    fetch('/upload-sequence', {
        ...
        if (data.status === 'success') {
            handleButtonState(button, [buttonSuccessClass, buttonDisabledClass], [buttonDisabledClass, buttonWarningClass], 'Uploaded')
            uploadFinished = true
